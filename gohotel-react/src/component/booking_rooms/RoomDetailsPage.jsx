...
const RoomDetailsPage = () => {
  ...
  const [numberOfAdults, setNumberOfAdults] = useState(1); // State variable for number of adults
  const [numberOfChildren, setNumberOfChildren] = useState(0); // State variable for number of children
  ...
  const handleGoBack = () => {
    setShowDatePicker(false); // hidden
    navigate('/rooms'); // Navigate to rooms
  };
  ...
  return (
    <div className="room-details-booking">
      ...
      {bookings && bookings.length > 0 && (
        <div>
          <h3>{t('roomDetails.existingBooking')}</h3>
      ...
      <div className="booking-info">
        <button className="book-now-button" onClick={() => setShowDatePicker(true)}>{t('roomDetails.bookNow')}</button>
        <button className="go-back-button" onClick={handleGoBack}>{t('roomDetails.goBack')}</button>
        {showDatePicker && (
          <div className="date-picker-container">
            ...
            <div className='guest-container'>
              <div className="guest-div">
                <label>{t('roomDetails.bookingInfo.numberOfAdults')}:</label>
                <input
                  type="number"
                  min="1"
              ...
              <div className="guest-div">
                <label>{t('roomDetails.bookingInfo.numberOfChildren')}:</label>
                <input
                  type="number"
                  min="0"
              ...
              <button className="confirm-booking" onClick={handleConfirmBooking}>{t('roomDetails.confirmBookingButton')}</button>
            </div>
          </div>
        )}
        {totalPrice > 0 && (
