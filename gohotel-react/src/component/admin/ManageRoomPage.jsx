...
const ManageRoomPage = () => {
  ...
  return (
    <div className='all-rooms'>
      <h2>{t('admin.manageRooms.allRooms')}</h2>
      <div className='all-room-filter-div' style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div className='filter-select-div'>
          <label>{t('admin.manageRooms.filter')}</label>
          <select value={selectedRoomType} onChange={handleRoomTypeChange}>
            <option value="">{t('admin.manageRooms.filterAll')}</option>
            {roomTypes.map((type) => (
          ...
          <button className='add-room-button' onClick={() => navigate('/admin/add-room')}>
            {t('admin.manageRooms.addRoom.title')}
          </button>
        </div>
      </div>
